...
import { User, Briefcase, FileText, Key, Upload, Calendar,Camera,Plus,Pencil
    // BriefcaseBusiness,
    // BriefcaseBusiness,
    // CreditCard
 } from 'lucide-react';
...
export default function EditEmployee() {
      ...
      const {
           ...
           onSubmit: async (values) => {
             try {
              ...
              const response = await dispatch(editEmployee(values))
        ...
        //     const fieldsToValidate = tabFieldsMap[currentTabId];
          
        //     try {
        //       await schema.validateAt(fieldsToValidate, values, { abortEarly: false });
        //       setActiveTab(tabs[currentIndex + 1].id);
        //     } catch (validationError) {
        //       // You can trigger Formik's setErrors manually if needed
         ...
         const fetchEmployee = async ()=>{
                ...
                if(status ===200){
                    ...
                    setValues({
                        ...
                        familyMember: data?.data?.familyMember || '',
                        ...
                        professionalEmail: data?.data?.professionalEmail || '',
                        branch: data?.data?.branch?.map(b => b._id) || [],
                        designation: data?.data?.designation?._id || '',
                        workingDays: data?.data?.workingDays || '',
                        workingHours: data?.data?.workingHours || '',
                        dateOfJoin: data?.data?.dateOfJoin || null,
                        dateOfLeave: data?.data?.dateOfLeave || null,
                        officeLocation: data?.data?.officeLocation || '',
                        salary: data?.data?.salary || '',
                        salaryStartDate: data?.data?.salaryStartDate || null,
                        salaryEndDate: data?.data?.salaryEndDate || null,
                        ...
                        IBAN: data?.data?.IBAN || '',
                        attendenceLoacation: data?.data?.attendenceLoacation || '',
                        bioMetricIp: data?.data?.bioMetricIp || '',
                        appointmentLetter: data?.data?.appointmentLetter || [],
                        salarySlip: data?.data?.salarySlip || [],
                        relivingLetter: data?.data?.relivingLetter || [],
                        experienceLetter: data?.data?.experienceLetter || [],
    ...
    return (
        <MainLayout>
            <ProfileHeader
            name="Edit Employee"
            breadcrumbs={["All Employees",`${values?.firstName} ${values?.lastName}`]}
            />
 ...
 {activeTab === 'personal' && (
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                <div>
                  ...
                  <select className="w-full p-2 pl-4 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 appearance-none bg-white  text-gray-400"
                    name="gender"
                    value={values.gender}
                    onChange={handleChange}
                    onBlur={handleBlur}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
                ...
                <div>
  <select
    className="w-full p-2 pl-4 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 text-gray-400"
    name="bloodgroup"
            ...
            <div className="p-2 mb-8">
              ...
              <div className="mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <select
                      className="w-full p-2 border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500 appearance-none bg-white  text-gray-400"
                      name="employeeType"
  ...
  const isPdf = uploadedFileUrl?.trim().toLowerCase().endsWith('.pdf');
  console.log(isPdf,"ispdff");
  ...
  <div className="border border-[#D9D9D9] rounded-[8px] p-6">
    ...
    <div className="flex justify-end mt-8 space-x-3">
      ...
      <button className="px-4 py-2 bg-[#2563EB] text-white rounded hover:bg-blue-600" onClick={handleNext}>
        ...
        {isEmployeeAddLoading ? <CircularProgress color='white'/> : `Update employee`}

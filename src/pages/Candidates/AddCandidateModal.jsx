...
const AddCandidateModal = ({ isOpen, onClose }) => {
  ...
  const {
    ...
    onSubmit:async (values) => {
      setLoading(true)
      try {
        const {data,status} = await api.addCandidate(values)
        if(status === 200){
          Toastify.success("candidated added successfully")
          resetForm()
          onClose()
        }
      } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
      <div className="bg-white w-full max-w-2xl rounded-xl shadow-xl px-6 py-4 relative">
        <h2 className="text-lg font-semibold text-[#14151C] mb-4">
          <span className="text-[#16151C]">Add New Candidate</span>
        </h2>

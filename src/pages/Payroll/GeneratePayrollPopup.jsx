...
const GeneratePayrollPopup = ({ open, handleClose }) => {
  ...
  const {
    ...
    onSubmit: async (values) => {
      try {
        setLoading(true);
        const { data, status } = await api.addPayRoll(values);
        if (status === 200) {
          dispatch(setPayrollRefresh());
          Toastify.success("Payroll added successfully");
          resetForm()
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      ...
      <DialogActions sx={{ justifyContent: "center", mb: 2 }}>
        ...
        >
          {loading ? <CircularProgress color="white" size={28}/> : "Add"}
        </Button>
      </DialogActions>
    </Dialog>
  );

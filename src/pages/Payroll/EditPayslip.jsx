    ...
    const formik = useFormik({
        ...
        onSubmit: async(values) => {
            ...
            try {
                ...
                const {data,status} = await api.updataAPaySlip(values)

                if(status === 200){
                    Toastify.success("Payslip Edited successfully")
                    dispatch(setRefresh())
        ...
        try {
            ...
            if(status === 200){
                ...
                formik.setValues({
                    ...
                    housingAllowence: data?.data?.housingAllowence || '',
                    transportAllowence: data?.data?.transportAllowence || '',
                    utilityAllowence: data?.data?.utilityAllowence || '',
                    productivityAllowence: data?.data?.productivityAllowence || '',
                    communicationAllowence: data?.data?.communicationAllowence || '',
                    inconvenienceAllowence: data?.data?.inconvenienceAllowence || '',
                    tax: data?.data?.tax?._id || '',
                    employeePension: data?.data?.employeePension || '',
                    grossSalary: data?.data?.grossSalary || 0,
                    netSalary: data?.data?.netSalary || 0,
                    month: data?.data?.month || '',
                    year: data?.data?.year || '',
                    totalDeduction: data?.data?.totalDeduction || 0,
    ...
    return (
        <MainLayout>
            ...
            <form onSubmit={formik.handleSubmit} className="px-6 py-4 max-w-6xl border border-[#D9D9D9] rounded-[8px] m-4">
                <h2 className="text-[16px] font-semibold mb-6">Edit Payslip</h2>
                ...
                <div className="mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <DatePicker
        selected={getDateFromMonthYear(formik.values.month, formik.values.year)}
      ...
      />
                ...
                <div className="flex gap-4">
                    <button type="submit" className="bg-blue-600 text-white px-6 py-2 rounded-md text-sm font-medium">
                       {
                        loading ? <CircularProgress color='white' size={25}/> : `Edit Payslip`               
                       } 
                    </button>
